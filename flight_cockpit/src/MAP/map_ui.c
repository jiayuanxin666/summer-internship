...
{
    SDL_Color white={255,255,255,255}, black={25,32,39,255}; SDL_Rect head={box.x,box.y,box.w,48};
    ...
    SDL_SetRenderDrawColor(ui->renderer,25,137,249,255); SDL_RenderFillRect(ui->renderer,&head); text(ui,ui->font_title,title,box.x+18,box.y+9,white);
    for(int i=0;i<count;i++){ SDL_Rect row={box.x+12,box.y+58+i*52,box.w-24,42}; SDL_SetRenderDrawColor(ui->renderer,189,195,199,255); SDL_RenderDrawRect(ui->renderer,&row); text(ui,ui->font,labels[i],row.x+10,row.y+9,(SDL_Color){85,92,99,255}); text(ui,ui->font,values[i],row.x+84,row.y+9,black); }
}
...
{
    int side=(int)(ui->width*.18f); if(side<280)side=280; SDL_Rect map={0,0,ui->width-side,ui->height};
    ...
    panel(ui,(SDL_Rect){map.w+12,24,side-24,226},"地点信息",place_labels,place_values,3);panel(ui,(SDL_Rect){map.w+12,270,side-24,330},"天气信息",weather_labels,weather_values,5);
    ui->fullscreen_button=(SDL_Rect){map.w+12,ui->height-82,side-24-132,58};ui->zoom_in_button=(SDL_Rect){ui->width-132,ui->height-82,58,58};ui->zoom_out_button=(SDL_Rect){ui->width-66,ui->height-82,58,58};icon_button(ui,ui->fullscreen_button,ui->fullscreen?ui->window_texture:ui->fullscreen_texture,ui->fullscreen?"窗口":"全屏");icon_button(ui,ui->zoom_in_button,ui->add_texture,"+");icon_button(ui,ui->zoom_out_button,ui->sub_texture,"−");
